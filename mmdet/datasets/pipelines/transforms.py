...
@PIPELINES.register_module
class RandomRotateBox(object):
    def __init__(self, roteta_type='value', rotate_angle=[0, 90, -90, 180]):
        if roteta_type not in ['vaule', ]:
            assert 'rotate_type shoule be in value and range'
        self.roteta_type = roteta_type
        self.rotate_angle = rotate_angle
    def _rotate_image(self, img, rotate_angle):
        if rotate_angle == -90:
            rotate_image = np.flip(img.transpose(1, 0, 2), 1)
        elif rotate_angle == 90:
            rotate_image = np.flip(img.transpose(1, 0, 2), 0)
        elif rotate_angle == 180 or rotate_angle == -180:
            rotate_image = np.flip(np.flip(img, 1), 0)
        elif rotate_angle == 0:
            rotate_image = img
        else:
            raise NotImplementedError
        return rotate_image
    def _rotate_box(self, gt_rbboxes_points, rotate_angle, img):
        height, width, _ = img.shape
        rotate_gt_rbboxes_points = np.zeros_like(gt_rbboxes_points)
        if rotate_angle == -90:
            temp = gt_rbboxes_points[:, 0::2].copy()
            rotate_gt_rbboxes_points[:, 0::2] = width - gt_rbboxes_points[:, 1::2]
            rotate_gt_rbboxes_points[:, 1::2] = temp
            return rotate_gt_rbboxes_points
        elif rotate_angle == 90:
            temp = gt_rbboxes_points[:, 0::2].copy()
            rotate_gt_rbboxes_points[:, 0::2] = gt_rbboxes_points[:, 1::2]
            rotate_gt_rbboxes_points[:, 1::2] = height - temp
            return rotate_gt_rbboxes_points
        elif rotate_angle == 180 or rotate_angle == -180:
            rotate_gt_rbboxes_points[:, 0::2] = width - gt_rbboxes_points[:, 0::2].copy()
            rotate_gt_rbboxes_points[:, 1::2] = height - gt_rbboxes_points[:, 1::2].copy()
            return rotate_gt_rbboxes_points
        elif rotate_angle == 0:
            return gt_rbboxes_points
        else:
            raise NotImplementedError
    def __call__(self, results):
        if self.roteta_type == 'value':
            angle_num = len(self.rotate_angle)
            assert angle_num > 0, 'rotate_angle shoule be 1 or more'
            angle_id = np.random.randint(angle_num)
            rotate_angle = self.rotate_angle[angle_id]
        else:
            raise NotImplementedError
        img = results['img']
        gt_rbboxes_points = results['gt_bboxes']
        rotate_img = self._rotate_image(img, rotate_angle)
        #         rotate_gt_rbboxes_points = self._rotate_box(gt_rbboxes_points, rotate_matrix)
        rotate_gt_rbboxes_points = self._rotate_box(gt_rbboxes_points, rotate_angle, img)

        results['img'] = rotate_img.astype(np.uint8)
        results['gt_bboxes'] = rotate_gt_rbboxes_points.astype(np.float32)

        return results

@PIPELINES.register_module
class AddParams(object):
    def __init__(self, with_xyxy=True, with_xywht=True, remove_small=True, just_remove_small=True):
        self.with_xyxy = with_xyxy
        self.with_xywht = with_xywht
        self.remove_small = remove_small
        self.just_remove_small = just_remove_small

    def _get_rbboxes_rects(self, gt_rbboxes_points):
        rbbox_xs = gt_rbboxes_points[:, 0::2]
        rbbox_ys = gt_rbboxes_points[:, 1::2]
        xmin = rbbox_xs.min(1)[:, None]
        ymin = rbbox_ys.min(1)[:, None]
        xmax = rbbox_xs.max(1)[:, None]
        ymax = rbbox_ys.max(1)[:, None]
        gt_rbboxes_rects = np.hstack([xmin, ymin, xmax, ymax])
        return gt_rbboxes_rects

    def _get_rbboxes_xywht(self, gt_rbboxes_points):
        gt_bboxes_xywht = []
        for rbbox_points in gt_rbboxes_points:
            rbbox_points_4x2 = rbbox_points.astype(np.int64).reshape(4, 2)
            rbbox_xywht = cv2.minAreaRect(rbbox_points_4x2)
            x_ctr, y_ctr, width, height, theta = rbbox_xywht[0][0], rbbox_xywht[0][1], \
                                                 rbbox_xywht[1][0], rbbox_xywht[1][1], rbbox_xywht[2]
            gt_bbox_xywht = np.array([x_ctr, y_ctr, width, height, theta / 180 * np.pi])
            gt_bboxes_xywht.append(gt_bbox_xywht)
        return np.array(gt_bboxes_xywht)

    def _remove_small(self, gt_bboxes_xywht):
        keep_ind = (gt_bboxes_xywht[:, 2] > 1) & (gt_bboxes_xywht[:, 3] > 1)

        return keep_ind

    def __call__(self, results):
        if self.just_remove_small:
            gt_rbboxes_points = results['gt_bboxes']
            gt_rbboxes_xywht = self._get_rbboxes_xywht(gt_rbboxes_points)
            keep_ind = self._remove_small(gt_rbboxes_xywht)
            results['gt_bboxes'] = results['gt_bboxes'][keep_ind]
            results['gt_labels'] = results['gt_labels'][keep_ind]
        if self.with_xyxy:
            gt_rbboxes_points = results['gt_bboxes']
            gt_rbboxes_rects = self._get_rbboxes_rects(gt_rbboxes_points)
            results['gt_bboxes_rects'] = gt_rbboxes_rects.astype(np.float32)
        if self.with_xywht:
            gt_rbboxes_points = results['gt_bboxes']
            gt_rbboxes_xywht = self._get_rbboxes_xywht(gt_rbboxes_points)
            results['gt_bboxes_xywht'] = gt_rbboxes_xywht.astype(np.float32)
        if self.remove_small:
            gt_rbboxes_xywht = results['gt_bboxes_xywht']
            keep_ind = self._remove_small(gt_rbboxes_xywht)
            results['gt_bboxes'] = results['gt_bboxes'][keep_ind]
            results['gt_bboxes_rects'] = results['gt_bboxes_rects'][keep_ind]
            results['gt_bboxes_xywht'] = results['gt_bboxes_xywht'][keep_ind]
        return results
